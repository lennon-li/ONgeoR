...
dataset (`inst/extdata/hive.rds`), accessed via
[`retrieve_hive()`](https://lennon-li.github.io/ONgeoR/reference/retrieve_hive.md)
or `retrieve_source("hive")`, rather than fetched live like the
LIO-backed `retrieve_*()` functions.

The version shipped with the package has been simplified
(`sf::st_simplify(dTolerance = 100, preserveTopology = TRUE)`, applied
in the source's native projected/meter CRS) and reprojected to EPSG:4326
for interactive (leaflet) mapping. The full-resolution source shapefile
is maintained by the author outside the package and is not distributed
here.
...
- Level:
  ...
  Hierarchy level of the grid cell (1, 2, or 3).

- HIVE_ID:

  HIVE identifier for the grid cell.

- geometry:

  `MULTIPOLYGON` geometry, EPSG:4326.
